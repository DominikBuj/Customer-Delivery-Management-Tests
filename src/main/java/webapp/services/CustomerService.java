...
public enum CustomerService {
	...
	public CustomerDTO getCustomerByVat (int vat, CustomerFinder customerFinder) throws ApplicationException {
		if (!isValidVAT (vat))
	...
	public void addCustomer(CustomerRowDataGateway customer) throws ApplicationException {
		if (!isValidVAT(customer.getVAT()))
			throw new ApplicationException ("Invalid VAT number: " + customer.getVAT());
		else try {
			customer.insert();
			
		} catch (PersistenceException e) {
				throw new ApplicationException ("Can't add customer with vat number " + customer.getVAT() + ".", e);
		}
	}
	...
	public void updateCustomerPhone(int vat, int phoneNumber, CustomerFinder customerFinder) throws ApplicationException {
		if (!isValidVAT (vat))
			throw new ApplicationException ("Invalid VAT number: " + vat);
		else try {
			CustomerRowDataGateway customer = customerFinder.getCustomerByVATNumber(vat);
	...
	public void removeCustomer(int vat, CustomerFinder customerFinder) throws ApplicationException {
		if (!isValidVAT (vat))
			throw new ApplicationException ("Invalid VAT number: " + vat);
		else try {
			CustomerRowDataGateway customer = customerFinder.getCustomerByVATNumber(vat);
